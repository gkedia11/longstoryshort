...
export function AuthPanel({ compact = false }: { compact?: boolean }) {
  ...
  const formSpacing = compact ? "space-y-2" : "space-y-5";
  const status = <p role="status" aria-live="polite" className={`${compact ? "mt-2 text-[11px] leading-4" : "mt-5 text-sm leading-6"} text-[#52615a]`}>{message}</p>;
  const field = (id: string, label: string, value: string, setValue: (value: string) => void, type = "password") => <div><label htmlFor={id} className={`${compact ? "text-[11px]" : "text-sm"} font-semibold text-[#101513]`}>{label}</label><input id={id} type={type} className={`field ${compact ? "field-compact mt-1 text-xs" : "mt-2"}`} value={value} onChange={(event) => setValue(event.target.value)} disabled={disabled} required minLength={type === "password" ? 8 : undefined} /></div>;
  const action = (label: string, icon: typeof KeyRound) => { const Icon = icon; return <button type="submit" disabled={disabled} className={`inline-flex w-full items-center justify-center gap-2 rounded-full bg-[#007a4d] font-semibold text-white transition hover:bg-[#004d33] disabled:cursor-not-allowed disabled:opacity-55 ${compact ? "min-h-9 px-3 py-2 text-xs" : "min-h-12 px-5 py-3"}`}><Icon aria-hidden="true" size={compact ? 16 : 18} />{label}</button>; };
